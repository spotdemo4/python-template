...
escaped_git_name=$(escape_string "$git_name")
escaped_git_email=$(escape_string "$git_email")
nix_description=$(printf '%s' "$escaped_description" | sed 's/${/\\${/g')

changes_started=true
old_slug=python-template
...
replace_literal "$old_description" "$escaped_description" pyproject.toml
replace_literal 'authors = [{ name = "spotdemo4", email = "me@trev.xyz" }]' "authors = [{ name = \"$escaped_git_name\", email = \"$escaped_git_email\" }]" pyproject.toml
